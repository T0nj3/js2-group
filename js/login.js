  ...
  .addEventListener("submit", async function (event) {
    ...
    try {
      ...
      if (result) {
        ...
        /** if there is no token, email or name, it thorw a error message (token, email or name is undefined)*/
        ...
        document.getElementById("message").textContent =
          "Login successful! Redirecting to feed page...";
        /** after 2 seconds, redirect to feedpage */
        setTimeout(() => {
          window.location.href = "../post/feedpage.html";
        }, 2000);
      }
      /** if result is false, it throws an error message (something went wrong. Please try again)*/
    } catch (error) {
